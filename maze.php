...
class Maze 
{
    ...
    */
    public function printOut($markX,$markY)
    {
        $this->log("Horizontal walls: %s", json_encode($this->horWalls));
        $this->log("Vertical walls: %s", json_encode($this->vertWalls));        
 
        $this->northDoor = 0; //mt_rand(0,$this->width-1);
        $this->eastDoor = $this->height-1; //mt_rand(0, $this->height-1);
 
        $str = '+';
        for ($i=0;$i<$this->width;$i++) {
            $str .= ($this->northDoor == $i) ? '   +' : '---+';
        }
        $str .= "\r\n";
        for ($i=0; $i<$this->height; $i++) {
            ...
            for ($j=0; $j<$this->width; $j++) {
                ...
                $t = (isset($this->vertWalls[$j][$i]) ? $this->vertWalls[$j][$i] : '|   ');
                if($markX == $i && $markY == $j){
                    $t[2] = "*";
                }
                $str .= $t;
            ...
            $str .= ($i == $this->eastDoor ? '  ' : '|')."\r\n".'+';
            for ($j=0; $j<$this->width; $j++) {
                $str .= (isset($this->horWalls[$j][$i]) ? $this->horWalls[$j][$i] : '---+');
            }
            $str .= "\r\n";
        }
        return $str;
